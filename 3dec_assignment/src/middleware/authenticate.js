...
module.exports = async(req, res, next)=>{
    ...
    // if not recieved or token is not a beare token then we will throw error
    ...
    if(!user) return res.status(400).json({status: "Failed", message: "please provide a valid token"});
